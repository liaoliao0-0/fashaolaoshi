...
**用法示例**：
- 单只查询：`python quote.py sh000001`
- 批量查询：`python quote.py sh000001 sz399001 sh600519 hkHSI`
...
**调用方式**：
```
execute_command: python {skill_dir}/quote.py <codes...>
```
...
flowchart TD
    subgraph 入口["【第一步】读取今日行情"]
        A["调用 quote.py<br/>python quote.py sh000300 hkHSI usNDX"] --> A2["缓存至会话上下文<br/>供后续判断使用"]
    end
...
**常用命令**：
- `python quote.py sh000300` — 沪深300
- `python quote.py hkHSI` — 恒生指数
- `python quote.py usNDX` — 纳斯达克100
- `python quote.py hf_GC` — 国际黄金（美元/盎司）
- `python quote.py hf_CL` — 国际原油（美元/桶）
- `python quote.py fut_ur2607` — 国内尿素期货（自动探测交易所）
- `python quote.py fut_m2509.dce` — 国内豆粕（大商所）
- `python quote.py fut_cu2506.shf` — 国内沪铜（上期所）
- `python quote.py fut_rb2506.shf` — 国内螺纹钢（上期所）
